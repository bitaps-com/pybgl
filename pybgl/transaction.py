...
class Transaction(dict):
    ...
    def __init__(self, raw_tx=None, format="decoded", version=1,
        ...
        if sw:
            self["segwit"] = True
            self["hash"] = sha3_256(self["rawTx"])
            self["txId"] = sha3_256(b"".join((self["rawTx"][:4], self["rawTx"][6:sw], self["rawTx"][-4:])))
        else:
            self["segwit"] = False
            self["txId"] = sha3_256(self["rawTx"])
            self["hash"] = self["txId"]
        if not keep_raw_tx:
            self["rawTx"] = None
    ...
    def commit(self):
        ...
        self["txId"] = sha3_256(no_segwit_view)
